...
import { Job } from "../interfaces/jobs.class";
  ...
  export class RecentSearchInterceptor implements NestInterceptor {
    ...
    intercept(context: ExecutionContext, next: CallHandler) {
      ...
      if (cachedRecentList.length) {
        ...
        const recentList = cachedRecentList.reduce<Job[]>((accList, cachedResult) => {
      ...
      return next.handle().pipe(
        ...
        tap((list: Job[]) => {
          if (token && query?.trim().length) {
            this.recentSearchService.addRecentSearch(token, query, list);
          }
        }),
        catchError((err) => {
